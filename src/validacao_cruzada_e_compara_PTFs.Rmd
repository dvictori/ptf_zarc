...
plot(dados$AD, fit.m2_destrans,
     xlab = 'AD obs (mm cm⁻¹)',
     ylab = 'AD modelo (mm cm⁻¹)',
     pch = 20)
points(dados$AD, AD_zarc_3solos, col = 'red',
       pch = 20)
...
ptfs <- data.frame(
  ad = dados_filtrados$AD,
  zarc = AD_independente_filtrado,
...
title(main = 'PTF Zarc',
      sub = paste('RMSE:', round(rmse(ptfs$ad, ptfs$zarc), 3),
                  'R²:', round(cor(ptfs$ad, ptfs$zarc)^2, 3),
                  'R:', round(cor(ptfs$ad, ptfs$zarc), 3)))
```
A PTF de Assad, baseada apenas em areia total, apresenta RMSE = `r round(rmse(ptfs$ad, ptfs$assad),3)` r  e r² = `r round(cor(ptfs$ad, ptfs$assad)^2, 3)`, com um claro patamar próximo a AD de 1,22 mm cm⁻¹.
...
title(main = 'PTF Assad',
      sub = paste('RMSE:', round(rmse(ptfs$ad, ptfs$assad), 3),
                  'R²:', round(cor(ptfs$ad, ptfs$assad)^2, 3),
                  'R:', round(cor(ptfs$ad, ptfs$assad), 3)))
```

A PTF de Giarola, baseada apenas em silte e argila, apresenta RMSE = `r round(rmse(ptfs$ad, ptfs$giarola),3)` e r² = `r round(cor(ptfs$ad, ptfs$giarola)^2, 3)`. Das PTFs que estamos usando na comparação, essa é a única em que é preciso estimar CC e PMP separadamente e calcular AD pela diferença.
...
title(main = 'PTF Giarola',
      sub = paste('RMSE:', round(rmse(ptfs$ad, ptfs$giarola), 3),
                  'R²:', round(cor(ptfs$ad, ptfs$giarola)^2, 3),
                  'R:', round(cor(ptfs$ad, ptfs$giarola), 3)))
```

A PTF de van den Berg, baseada apenas argila, apresenta RMSE = `r round(rmse(ptfs$ad, ptfs$van_den_berg),3)` e r² = `r round(cor(ptfs$ad, ptfs$van_den_berg)^2, 3)`. Por ser uma regressão linear em relação a argila, apresenta distribuição dos pontos semelhante ao obtido em Giarola (que se reduz a uma regressão em relação a argila). Porém, o coeficientes da regressão de van den Berg indicam pouca variação da AD em relação à argila, como pode ser visto no gráfico abaixo.
...
title(main = 'PTF van den Berg',
      sub = paste('RMSE:', round(rmse(ptfs$ad, ptfs$van_den_berg), 3),
                  'R²:', round(cor(ptfs$ad, ptfs$van_den_berg)^2, 3),
                  'R:', round(cor(ptfs$ad, ptfs$van_den_berg), 3)))
```
A PTF original do ZARC, que utiliza 3 classes de solo, apresenta RMSE = `r round(rmse(ptfs$ad, ptfs$zarc_orig), 3)` e r² = `r round(cor(ptfs$ad, ptfs$zarc_orig)^2, 3)`. 
...
title(main = 'PTF ZARC Original',
      sub = paste('RMSE:', round(rmse(ptfs$ad, ptfs$zarc_orig), 3),
                  'R²:', round(cor(ptfs$ad, ptfs$zarc_orig)^2, 3),
...
A verificação independente para todo o conjunto de dados mostrou que a PTF desenvolvida apresentou RMSE = `r round(rmse(dados_extra$AD, dados_extra$zarc), 3)` e r² = `r round(cor(dados_extra$AD, dados_extra$zarc)^2, 3)`

```{r}
plot(dados_extra$AD, dados_extra$zarc,
     xlab = 'AD obs (mm cm⁻¹)',
     ylab = 'AD modelo (mm cm⁻¹)')
abline(a = 0, b = 1, col = 'red')
title(main = 'PTF ZARC (verificação independente)',
      sub = paste('RMSE:', round(rmse(dados_extra$AD, dados_extra$zarc), 3),
                  'R²:', round(cor(dados_extra$AD, dados_extra$zarc)^2, 3),
...
dados_extra_filtrado$zarc <- (lambda * dados_extra_filtrado$zarc + 1) ^(1 / lambda)
...
Quando avaliada em relação aos dados independetes, a nova PTF criada apresenta RMSE = `r round(rmse(dados_extra$AD, dados_extra$zarc), 3)` e r² = `r round(cor(dados_extra$AD, dados_extra$zarc)^2, 3)`

```{r}
plot(dados_extra_filtrado$AD, dados_extra_filtrado$zarc,
     xlab = 'AD obs (mm cm⁻¹)',
     ylab = 'AD modelo (mm cm⁻¹)')
abline(a = 0, b = 1, col = 'red')
title(main = 'PTF ZARC (verificação independente)',
      sub = paste('RMSE:', round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$zarc), 3),
                  'R²:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$zarc)^2, 3),
                  'R:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$zarc), 3)))
```

Para a PTF de Assad, os coeficientes de ajuste foram: RMSE = `r round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$assad), 3)` e r² = `r round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$assad)^2, 3)`.

```{r}
plot(dados_extra_filtrado$AD, dados_extra_filtrado$assad,
     xlab = 'AD obs (mm cm⁻¹)',
     ylab = 'AD modelo (mm cm⁻¹)')
abline(a = 0, b = 1, col = 'red')
title(main = 'PTF Assad (verificação independente)',
      sub = paste('RMSE:', round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$assad), 3),
                  'R²:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$assad)^2, 3),
                  'R:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$assad), 3)))
```
Para a PTF de Giarola, os coeficientes de ajuste foram: RMSE = `r round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$giarola), 3)` e r² = `r round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$giarola)^2, 3)`.

```{r}
plot(dados_extra_filtrado$AD, dados_extra_filtrado$giarola,
     xlab = 'AD obs (mm cm⁻¹)',
     ylab = 'AD modelo (mm cm⁻¹)')
abline(a = 0, b = 1, col = 'red')
title(main = 'PTF Giarola (verificação independente)',
      sub = paste('RMSE:', round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$giarola), 3),
                  'R²:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$giarola)^2, 3),
                  'R:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$giarola), 3)))
```

Para a PTF de van den Berg, os coeficientes de ajuste foram: RMSE = `r round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$van_den_berg), 3)` e r² = `r round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$van_den_berg)^2, 3)`.

```{r}
plot(dados_extra_filtrado$AD, dados_extra_filtrado$van_den_berg,
     xlab = 'AD obs (mm cm⁻¹)',
     ylab = 'AD modelo (mm cm⁻¹)')
abline(a = 0, b = 1, col = 'red')
title(main = 'PTF van den Berg (verificação independente)',
      sub = paste('RMSE:', round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$van_den_berg), 3),
                  'R²:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$van_den_berg)^2, 3),
                  'R:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$van_den_berg), 3)))
```

E para a relação originalmente utilizada no ZARC, com 3 tipos de solos, temos: RMSE = `r round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$zarc_orig), 3)` e r² = `r round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$zarc_orig)^2, 3)`.

```{r}
plot(dados_extra_filtrado$AD, dados_extra_filtrado$zarc_orig,
     xlab = 'AD obs (mm cm⁻¹)',
     ylab = 'AD modelo (mm cm⁻¹)')
abline(a = 0, b = 1, col = 'red')
title(main = 'PTF Zarc Original (verificação independente)',
      sub = paste('RMSE:', round(rmse(dados_extra_filtrado$AD, dados_extra_filtrado$zarc_orig), 3),
                  'R²:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$zarc_orig)^2, 3),
                  'R:', round(cor(dados_extra_filtrado$AD, dados_extra_filtrado$zarc_orig), 3)))
```
